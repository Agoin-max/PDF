import pandas as pd
from Supplier_ShiPing.GetAExcel import FormationDocuments


class ShiPingComparison:
    ...
    def info_judge(self):
        ...
        for index1, row1 in df.iterrows():
            ...
            for index2, row2 in cn.iterrows():
                if row1["Invo"] == row2["Invo"] and row1["Item"] == row2["Item"] and row1["P/O"] == \
                        row2["P/O"] and row1["QUANTITY"] == row2["QUANTITY"]:
                    isExist = True
                    break
            if not isExist:
                Error_Info.append("Invo:{},品牌：{}，型号：{},数量不一致！".format(row1["Invo"], row1["Item"], row1["P/O"]))
...
# print(ShiPingComparison().info_judge())
