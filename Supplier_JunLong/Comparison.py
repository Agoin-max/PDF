...
import pandas as pd
from Supplier_JunLong.GetAExcel import FormationDocuments


class JunLongComparison:
    ...
    def info_judge(self):
        ...
        for index1, row1 in df.iterrows():
            ...
            for index2, row2 in cn.iterrows():
                if row1["Invo"] == row2["Invo"] and row1["PN"] == row2["PN"] and row1["QTY"] == row2[
                    "QTY"]:
                    isExist = True
                    break
            if not isExist:
...
# print(JunLongComparison().info_judge())
