import pandas as pd
from Supplier_HongYi.GetAExcel import FormationDocuments


class HongYiComparison:
    ...
    def info_judge(self):
        ...
        for index1, row1 in df.iterrows():
            ...
            for index2, row2 in cn.iterrows():
                if row1["Invo"] == row2["Invo"] and row1["DESCRIPTION"] == row2["DESCRIPTION"] and row1["QTY"] == row2[
                    "QTY"]:
                    isExist = True
                    break
            if not isExist:
                Error_Info.append("Invo:{},型号:{},数量不一致！".format(row1["Invo"], row1["DESCRIPTION"]))
...
# print(HongYiComparison().info_judge())
