...
class ExtractHongYiPdf(DataPdf):
        ...
        for r in range(self.pdf_pages):
            ...
            for item in dict_words:
                if item["text"] == "INVOICE":
                    c = "INVOICE"
                    pattarn = re.compile(r"Invoice:(.*?)\n", re.S)
                    text_list = re.findall(pattarn, text_words)
                    Invo = text_list[0].replace(" ", "")

                    pattarn = re.compile(r"(USD)(.*?)TOTAL", re.S)
                    text_list = re.findall(pattarn, text_words)
                    # print(text_list[0][1].split("\n")[1:-1])

                    for item in text_list[0][1].split("\n")[1:-1]:
                        if item.find("PCS") >= 0:
                            Brand.append(item.split(" ")[1])
                            DESCRIPTION.append(item.split(" ")[2])
                            QTY.append(item.split(" ")[4])
                            Price.append(item.split(" ")[-3])

                    pattarn = re.compile(r"PO:(.*?)\n", re.S)
                    text_list = re.findall(pattarn, text_words)
                    PO = text_list

                    pattarn = re.compile(r"PARTNO:(.*?)\n", re.S)
                    text_list = re.findall(pattarn, text_words)
                    PART = text_list
                    ...
                    pattarn = re.compile(r"manufacture : (.*?)\n", re.S)
                    text_list = re.findall(pattarn, text_words)
                    COO = text_list[0].replace(" ", "")
