...
from Supplier_Samtec.Modify_Information import ModifyData
    ...
    def enter_info(self, df, ins, name, omg):
        for i in range(len(df)):
            data = {"商品类型": "",
                    ...
                    "件数": df.iloc[i]["CTN"],
                    "净重": df.iloc[i]["NW"].replace("Kg", ""),
                    "毛重": df.iloc[i]["GW"].replace("Kg", ""),
                    "税号": "",
                    "SKU": "",
                    "供应商": name,
                    "期票天数": "",
            ...
            for c in range(len(ins)):
                if df.iloc[i]["Invo"] == ins.iloc[c]["Invo"] and df.iloc[i]["PN"] == ins.iloc[c][
                    ...
                    data["报关单价"] = ins.iloc[c]["UNIT_PRICE"]
                    data["对应的采购"] = ins.iloc[c]["CUSTOM_PART"]
