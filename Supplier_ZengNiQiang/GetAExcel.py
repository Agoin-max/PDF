...
from Supplier_ZengNiQiang.Modify_Information import ModifyData
    ...
    def enter_info(self, df, ins, name, omg):
        for i in range(len(df)):
            ...
            for c in range(len(ins)):
                if df.iloc[i]["Invo"].strip() == ins.iloc[c]["Invo"].strip() and df.iloc[i]["PN"].strip() == \
                        ins.iloc[c][
                            "PN"].strip():
                    data["商品小类"] = ins.iloc[c]["DESCRIPTION"]
                    data["品牌"] = ins.iloc[c]["BRAND"]
                    data["报关单价"] = ins.iloc[c]["PRICE"]
                    data["对应的采购"] = ins.iloc[c]["PART_NO"]
